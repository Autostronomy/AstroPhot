...
class Zernike_Star(Star_Model):
    ...
    def initialize(self, target=None, parameters=None, **kwargs):
        ...
        if parameters["Anm"].value is not None:
            assert len(self.nm_list) == len(parameters["Anm"].value), "nm_list must match coefficients (Anm)"
            return

        # Set the default coefficients to zeros
        parameters["Anm"].set_value(torch.zeros(len(self.nm_list)), override_locked = True)
    ...
    def coefficients(n, m):
        ...
        for k in range(int((n - abs(m))/2)+1):
            C.append((k, (-1)**k * binom(n - k, k) * binom(n - 2*k, (n-abs(m))/2 - k)))
        return C
