...
def build_pipeline(**options):
    ...
    if 'ap_pipeline_main' in options:
        ap_pipeline['structure'] = list(options['ap_pipeline_main'])
    ...
    if "ap_pipeline_replace_steps" in options:
        for step in options["ap_pipeline_replace_steps"]:
            pipe_index = ap_pipeline['structure'].index(step[0])
            ap_pipeline['structure'][pipe_index] = step[1]
    ...
    if "ap_pipeline_fitloop_replace_steps" in options:
        for step in options["ap_pipeline_fitloop_replace_steps"]:
            pipe_index = ap_pipeline['node_kwargs']['fit_loop']["structure"].index(step[0])
            ap_pipeline['node_kwargs']['fit_loop']["structure"][pipe_index] = step[1]
