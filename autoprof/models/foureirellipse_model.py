...
class FourierEllipse_Galaxy(Galaxy_Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.modes = torch.tensor(kwargs.get("modes", (1, 3, 4)), dtype = AP_config.ap_dtype, device = AP_config.ap_device)
    ...
    def initialize(self, target=None, parameters=None, **kwargs):
        ...
        if parameters["am"].value is None:
            parameters["am"].set_value(torch.zeros(len(self.modes), dtype = AP_config.ap_dtype, device = AP_config.ap_device), override_locked=True)
        if parameters["am"].uncertainty is None:
            parameters["am"].set_uncertainty(
                torch.tensor(0.05 * np.ones(len(self.modes)), dtype = AP_config.ap_dtype, device = AP_config.ap_device), override_locked=True
            )
        if parameters["phim"].value is None:
            parameters["phim"].set_value(torch.zeros(len(self.modes), dtype = AP_config.ap_dtype, device = AP_config.ap_device), override_locked=True)
        if parameters["phim"].uncertainty is None:
            parameters["phim"].set_uncertainty(
                torch.tensor((5 * np.pi / 180) * np.ones(len(self.modes)), dtype = AP_config.ap_dtype, device = AP_config.ap_device),
