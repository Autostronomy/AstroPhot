...
class SingleRay_Galaxy(Galaxy_Model):
    ...
    model_type = f"singleray {Galaxy_Model.model_type}"
    parameter_specs = {
        "q": {"units": "b/a", "limits": (0,1), "uncertainty": 0.03},
        "PA": {"units": "rad", "limits": (0,2*np.pi), "cyclic": True, "uncertainty": 0.06},
    }
    ...
    def polar_model(self, R, T, image):
        model = torch.zeros(R.shape)
        for r in range(self.rays):
            angles = (T - (self["PA"].value + r*2*np.pi/self.rays)) % (2*np.pi)
            indices = torch.logical_or(angles < (2*np.pi/self.rays), angles >= (2*np.pi*(1 - 1/self.rays)))
            weight = (torch.cos(angles[indices] * self.rays / 2) + 1)/2
            model[indices] += weight * self.iradial_model(r, R[indices], image)
        return model
