...
class TestNode(unittest.TestCase):
    ...
    def test_node_access(self):
        ...
        self.assertEqual(
            node1["node1"],
            node1,
            "node should get itself when getter called with its name",
        )
...
class TestParameter(unittest.TestCase):
    ...
    def test_parameter_state(self):
        ...
        P = Parameter_Node(
            "state", value=1.0, uncertainty=0.5, limits=(-2, 2), locked=True, prof=1.0
        )
        ...
        self.assertEqual(
            P.limits[0].tolist(), P2.limits[0].tolist(), "state should preserve limits"
        )
        self.assertEqual(
            P.limits[1].tolist(), P2.limits[1].tolist(), "state should preserve limits"
        )
    ...
    def test_parameter_value(self):
        ...
        P1 = Parameter_Node(
            "test1", value=0.5, uncertainty=0.5, limits=(-1, 1), locked=False, prof=1.0
        )
...
class TestParamContext(unittest.TestCase):
    def test_unlock(self):
        ...
        locked_param.value = 2.0
        self.assertEqual(
            locked_param.value.item(),
            1.0,
            "locked parameter should not be updated out of context",
        )
        ...
        self.assertEqual(
            locked_param.value.item(),
            2.0,
            "locked parameter should be updated in context",
        )
        with Param_Unlock():
            locked_param.value = 3.0
        self.assertEqual(
            locked_param.value.item(),
            3.0,
            "locked parameter should be updated in global unlock context",
        )
...
class TestParameterVector(unittest.TestCase):
    def test_param_vector_creation(self):
        ...
        P2 = Parameter_Node("test2", value=2.0, uncertainty=5.0, locked=False)
        P3 = Parameter_Node("test3", value=[4.0, 5.0], uncertainty=[5.0, 5.0], locked=False)
        ...
        self.assertTrue(
            torch.all(
                PG.vector_values()
                == torch.tensor([0.5, 2.0, 4.0, 5.0], dtype=P1.value.dtype, device=P1.value.device)
            ),
            "Vector store all leaf node values",
        )
        self.assertEqual(PG.mask.numel(), 4, "Vector should take all/only leaf node masks")
        self.assertEqual(
            PG.vector_identities().size,
            4,
            "Vector should take all/only leaf node identities",
        )
        ...
        PG.value = [1.0, 2.0, 3.0, 4.0]
        self.assertTrue(
            torch.all(
                PG.vector_values()
                == torch.tensor([1.0, 2.0, 3.0, 4.0], dtype=P1.value.dtype, device=P1.value.device)
            ),
            "Vector store all leaf node values",
        )
    ...
    def test_vector_masking(self):
        ...
        P3 = Parameter_Node("test3", value=[4.0, 5.0], uncertainty=[5.0, 3.0], locked=False)
        P4 = Parameter_Node("test4", value=P2)
        P5 = Parameter_Node("test5", value=lambda P: P["test1"].value ** 2, link=(P1,))
        PG = Parameter_Node("testgroup", link=(P1, P2, P3, P4, P5))
        ...
        with Param_Mask(PG, mask):
            self.assertTrue(
                torch.all(
                    PG.vector_values()
                    == torch.tensor([0.5, 5.0], dtype=P1.value.dtype, device=P1.value.device)
                ),
                "Vector store all leaf node values",
            )
            self.assertTrue(
                torch.all(
                    PG.vector_uncertainty()
                    == torch.tensor([0.3, 3.0], dtype=P1.value.dtype, device=P1.value.device)
                ),
                "Vector store all leaf node uncertainty",
            )
            self.assertEqual(
                PG.vector_mask().numel(),
                4,
                "Vector should take all/only leaf node masks",
            )
            self.assertEqual(
                PG.vector_identities().size,
                2,
            ...
            with Param_Mask(PG, new_mask):
                self.assertTrue(
                    torch.all(
                        PG.vector_values()
                        == torch.tensor([0.5], dtype=P1.value.dtype, device=P1.value.device)
                    ),
                    "Vector store all leaf node values",
                )
                self.assertTrue(
                    torch.all(
                        PG.vector_uncertainty()
                        == torch.tensor([0.3], dtype=P1.value.dtype, device=P1.value.device)
                    ),
                    "Vector store all leaf node uncertainty",
                )
                self.assertEqual(
    ...
    def test_vector_representation(self):
        ...
        self.assertEqual(
            ...
            8,
            "representation should collect all values",
        )
        with Param_Mask(PG, mask):
            # round trip
            ...
            self.assertTrue(
                torch.all(vec == PG.vector_values()),
...
test6: [[5.0, 6.0], [7.0, 8.0]] +- [[0.0, 0.0], [0.0, 0.0]] [none], limits: (None, [[10.0, 10.0], [10.0, 10.0]])""",
  ...
  test6 (id-140071931415616): [[5.0, 6.0], [7.0, 8.0]] +- [[0.0, 0.0], [0.0, 0.0]] [none], limits: (None, [[10.0, 10.0], [10.0, 10.0]])"""
    ...
    def test_empty_vector(self):
        ...
        P1 = Parameter_Node(
            "test1", value=0.5, uncertainty=0.3, limits=(-1, 1), locked=True, prof=1.0
        )
        P2 = Parameter_Node("test2", value=2.0, uncertainty=1.0, locked=True)
        P3 = Parameter_Node(
            "test3",
            value=[4.0, 5.0],
            uncertainty=[5.0, 3.0],
        ...
        self.assertEqual(
            PG.identities.shape,
            (0,),
            "all locked parameter should have empty identities",
        )
        self.assertEqual(
            PG.vector_names().shape,
            (0,),
            "all locked parameter should have empty names",
        )
        self.assertEqual(
            PG.vector_identities().shape,
            (0,),
            "all locked parameter should have empty identities",
        )

        self.assertEqual(
            PG.vector_values().shape,
            (0,),
            "all locked parameter should have empty values",
        )
        self.assertEqual(
            PG.vector_uncertainty().shape,
            (0,),
            "all locked parameter should have empty uncertainty",
        )
        self.assertEqual(
            PG.vector_mask().shape, (0,), "all locked parameter should have empty mask"
        )
        self.assertEqual(
            PG.vector_representation().shape,
            (0,),
            "all locked parameter should have empty representation",
        )
    ...
    def test_none_uncertainty(self):
        ...
        P2 = Parameter_Node("test2", value=2.0, locked=True)
